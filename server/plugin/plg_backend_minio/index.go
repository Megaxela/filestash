...
func (this MinioBackend) LoginForm() Form {
	return Form{
		Elmnts: []FormElement{
			FormElement{
				Name:  "type",
...
func (this MinioBackend) Rm(path string) error {
	...
	jobChan := make(chan MinioPath, this.threadSize)
	...
	err := client.ListObjectsV2PagesWithContext(
		...
		func(objs *s3.ListObjectsV2Output, lastPage bool) bool {
			...
			for _, object := range objs.Contents {
				jobChan <- MinioPath{p.bucket, *object.Key}
			}
			return aws.BoolValue(objs.IsTruncated)
		},
	)
...
func (this MinioBackend) createSession(bucket string) *session.Session {
     Log.Info("[minio] Create Session");
	newParams := map[string]string{"bucket": bucket}
	for k, v := range this.params {
		newParams[k] = v
	}
	c := MinioCache.Get(newParams)
	if c == nil {
		...
		MinioCache.Set(newParams, this.config.Region)
	} else {
